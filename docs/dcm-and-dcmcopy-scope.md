# `dcm` vs `dcmCopy`, and the scope of a nested tag
...
attribute *other* than the one being processed must read it from `dcmCopy`, or the answer
...
therefore received a mismatched pair, and everything reading `dcmCopy` resolved against the
top level:

- `ExprAction` seeds its `stringValue` from `dcmCopy.getString(tag)` — for a nested tag
...
against the dataset being de-identified. It now takes `dcmCopy`:
...
`AddTag`'s SOP-class check moved to `dcmCopy` for the same reason, and so did the
...
- `ProfileItem.getAction` — the `dcm` / `dcmCopy` contract, the ascending-order hazard, and
...
| `Expression` | `new ExprAction(tag, dcm.getVR(tag), dcmCopy)` | consistent — the VR of the current tag from `dcm`, everything else from the copy |
| `ActionDates` | value from `dcm`, referenced tags from `dcmCopy` | fixed, issue 1 |
| `ReplaceApi` | `parseArguments(dcmCopy)` | fixed, issue 1 |
| `AddTag` | latch and SOP class from `dcmCopy` | fixed, issue 3 |
| `AddPrivateTag` | latch from `dcmCopy`, private creator from `dcm` | fixed, issue 3 |

`Profile` itself is consistent: `ExprCondition` is built on `dcmCopy`, and the mask and
defacing lookups (`StationName`, `Columns`, `Rows`) read `dcmCopy`.

One theoretical divergence is left alone: `ExprAction` seeds its `stringValue` from
`dcmCopy.getString(tag)` while the contract says the value of the tag itself comes from
